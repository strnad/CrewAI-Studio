...
class MyCrew:
    ...
    def is_valid(self, show_warning=False):
        if len(self.agents) == 0:
            if show_warning:
                st.warning(t('crews.warning_no_agents', name=self.name))
            return False
        if len(self.tasks) == 0:
            if show_warning:
                st.warning(t('crews.warning_no_tasks', name=self.name))
            return False
        ...
        if self.process == Process.hierarchical and not (self.manager_llm or self.manager_agent):
            if show_warning:
                st.warning(t('crews.warning_no_manager', name=self.name))
            return False
        if self.planning and not self.planning_llm:
            if show_warning:
                st.warning(t('crews.warning_no_planning_llm', name=self.name))
            return False
        return True
    ...
    def draw(self,expanded=False, buttons=True):
        ...
        if self.edit:
            with st.container(border=True):
                st.text_input(t('crews.name_help'), value=self.name, key=name_key, on_change=self.update_name)
                st.selectbox(t('crews.process'), options=[Process.sequential, Process.hierarchical], index=[Process.sequential, Process.hierarchical].index(self.process), key=process_key, on_change=self.update_process)
                st.multiselect(t('crews.select_agents'), options=[agent.role for agent in ss.agents], default=[agent.role for agent in self.agents], key=agents_key, on_change=self.update_agents)
                ...
                st.multiselect(t('crews.select_tasks'), options=available_task_ids, default=default_task_ids, format_func=lambda x: next(task.description for task in ss.tasks if task.id == x), key=tasks_key, on_change=self.update_tasks)
                st.selectbox(t('crews.manager_llm'), options=[t('crews.none')] + llm_providers_and_models(), index=0 if self.manager_llm is None else llm_providers_and_models().index(self.manager_llm) + 1, key=manager_llm_key, on_change=self.update_manager_llm, disabled=(self.process != Process.hierarchical))
                st.selectbox(t('crews.manager_agent'), options=[t('crews.none')] + [agent.role for agent in ss.agents], index=0 if self.manager_agent is None else [agent.role for agent in ss.agents].index(self.manager_agent.role) + 1, key=manager_agent_key, on_change=self.update_manager_agent, disabled=(self.process != Process.hierarchical))
                st.checkbox(t('crews.verbose'), value=self.verbose, key=verbose_key, on_change=self.update_verbose)
                st.checkbox(t('crews.memory'), value=self.memory, key=memory_key, on_change=self.update_memory)
                st.checkbox(t('crews.cache'), value=self.cache, key=cache_key, on_change=self.update_cache)
                st.checkbox(t('crews.planning'), value=self.planning, key=planning_key, on_change=self.update_planning)
                st.selectbox(t('crews.planning_llm'), options=[t('crews.none')] + llm_providers_and_models(), index=0 if self.planning_llm is None else llm_providers_and_models().index(self.planning_llm) + 1, key=planning_llm_key, on_change=self.update_planning_llm, disabled=not self.planning)
                st.number_input(t('crews.max_rpm'), value=self.max_rpm, key=max_rpm_key, on_change=self.update_max_rpm)  
                ...
                st.button(t('common.save'), on_click=self.set_editable, args=(False,), key=rnd_id())
        else:
            fix_columns_width()
            expander_title = t('crews.crew_label', name=self.name) if self.is_valid() else f"❗ {t('crews.crew_label', name=self.name)}"
            with st.expander(expander_title, expanded=expanded):
                st.markdown(f"**{t('crews.process')}:** {self.process}")
                if self.process == Process.hierarchical:
                    st.markdown(f"**{t('crews.manager_llm')}:** {self.manager_llm}")
                    st.markdown(f"**{t('crews.manager_agent')}:** {self.manager_agent.role if self.manager_agent else t('crews.none')}")
                st.markdown(f"**{t('crews.verbose')}:** {self.verbose}")
                st.markdown(f"**{t('crews.memory')}:** {self.memory}")
                st.markdown(f"**{t('crews.cache')}:** {self.cache}")
                st.markdown(f"**{t('crews.planning')}:** {self.planning}")
                if self.planning and self.planning_llm:
                    st.markdown(f"**{t('crews.planning_llm')}:** {self.planning_llm}")
                st.markdown(f"**{t('crews.max_rpm')}:** {self.max_rpm}")
                st.markdown(f"**{t('crews.tasks_title')}**")
                for i, task in enumerate([task for task in self.tasks if task.agent and task.agent.id in [agent.id for agent in self.agents]], 1):
                    with st.container(border=True):
                        ...
                        st.markdown(f"**{t('crews.agent_label')}** {task.agent.role if task.agent else t('crews.none')}")
                        tools_list = ", ".join([tool.name for tool in task.agent.tools]) if task.agent else t('crews.none')
                        st.markdown(f" **{t('crews.tools_label')}** {tools_list}")
                        st.markdown(f" **{t('crews.llm_label')}** {task.agent.llm_provider_model}")
                if self.knowledge_source_ids and 'knowledge_sources' in ss:
                    ...
                    st.markdown(f"**{t('crews.knowledge_sources')}:** {', '.join(source_names)}")
                if buttons:
                    col1, col2 = st.columns(2)
                    with col1:
                        st.button(t('common.edit'), on_click=self.set_editable, key=rnd_id(), args=(True,))
                    with col2:
                        # Instead of direct delete, open modal for cascade delete handling
                        st.button(t('common.delete'), on_click=self.request_delete_modal, key=rnd_id())
    ...
    def analyze_dependencies(self):
        ...
        for agent in self.agents:
            ...
            if used_in_crews:
                conflicts.append(t('crews.conflict_used_in_crews', crews=', '.join(used_in_crews)))
            ...
            if external_tasks:
                conflicts.append(t('crews.conflict_used_in_external_tasks', tasks=", ".join([t.description[:40] for t in external_tasks])))
        ...
        for task in self.tasks:
            ...
            if used_in_crews:
                conflicts.append(t('crews.conflict_shared_crews', crews=', '.join(used_in_crews)))
            ...
            if ref_tasks:
                conflicts.append(t('crews.conflict_referenced_context', tasks=", ".join([rt.description[:40] for rt in ref_tasks])))
    ...
    def draw_delete_dialog(self):
        ...
        if not hasattr(st, 'dialog'):
            st.error(t('crews.error_streamlit_version'))
            return

        @st.dialog(t('crews.delete_dialog_title', name=self.name))
        def _dlg():
            st.markdown(f"### {t('crews.delete_confirm')}")
            st.markdown(t('crews.delete_description'))
            st.markdown(t('crews.delete_conflict_notice'))

            st.markdown(f"#### {t('crews.delete_agents_section')}")
            for info in deps['agents']:
                agent = info['obj']
                conflict = len(info['conflicts']) > 0
                checkbox_key = f"del_agent_{agent.id}"
                label = f"{t('crews.agent_label')} {agent.role}"
                default_val = False if conflict else True
                st.checkbox(label, value=default_val if checkbox_key not in ss else ss[checkbox_key], key=checkbox_key, help=(t('crews.conflict_prefix') + " | ".join(info['conflicts'])) if conflict else None)

            st.markdown(f"#### {t('crews.delete_tasks_section')}")
            for info in deps['tasks']:
                task = info['obj']
                conflict = len(info['conflicts']) > 0
                checkbox_key = f"del_task_{task.id}"
                label = f"{t('crews.task_label')} {task.description[:60]}"
                default_val = False if conflict else True
                st.checkbox(label, value=default_val if checkbox_key not in ss else ss[checkbox_key], key=checkbox_key, help=(t('crews.conflict_prefix') + " | ".join(info['conflicts'])) if conflict else None)
            ...
            with col_a:
                if st.button(t('crews.delete_cancel')):
                    self.clear_delete_modal()
                    st.rerun()
            with col_b:
                if st.button(t('crews.delete_crew_only')):
                    self.delete()
                    self.clear_delete_modal()
                    st.rerun()
            with col_c:
                if st.button(t('crews.delete_crew_selected'), type="primary"):
