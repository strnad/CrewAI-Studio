        ...
        if self.edit:
            with st.expander(expander_title, expanded=True):
                with st.form(key=f'form_{self.id}' if key is None else key):
                    self.description = st.text_area(t('tasks.description'), value=self.description)
                    self.expected_output = st.text_area(t('tasks.expected_output'), value=self.expected_output)
                    self.agent = st.selectbox(t('tasks.agent'), options=ss.agents, format_func=lambda x: x.role, index=0 if self.agent is None else agent_options.index(self.agent.role))
                    self.async_execution = st.checkbox(t('tasks.async_execution'), value=self.async_execution)
                    self.context_from_async_tasks_ids = st.multiselect(t('tasks.context_async'), options=[task.id for task in ss.tasks if task.async_execution], default=self.context_from_async_tasks_ids, format_func=lambda x: [task.description[:120] for task in ss.tasks if task.id == x][0])
                    self.context_from_sync_tasks_ids = st.multiselect(t('tasks.context_sync'), options=[task.id for task in ss.tasks if not task.async_execution], default=self.context_from_sync_tasks_ids, format_func=lambda x: [task.description[:120] for task in ss.tasks if task.id == x][0])
            ...
            with st.expander(expander_title):
                st.markdown(f"**{t('tasks.description')}:** {self.description}")
                st.markdown(f"**{t('tasks.expected_output')}:** {self.expected_output}")
                st.markdown(f"**{t('tasks.agent')}:** {self.agent.role if self.agent else t('tasks.none')}")
                st.markdown(f"**{t('tasks.async_execution')}:** {self.async_execution}")
                st.markdown(f"**{t('tasks.context_async')}:** {', '.join([task.description[:120] for task in ss.tasks if task.id in self.context_from_async_tasks_ids]) if self.context_from_async_tasks_ids else t('tasks.none')}")
                st.markdown(f"**{t('tasks.context_sync')}:** {', '.join([task.description[:120] for task in ss.tasks if task.id in self.context_from_sync_tasks_ids]) if self.context_from_sync_tasks_ids else t('tasks.none')}")
