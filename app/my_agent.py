...
class MyAgent:
    ...
    def draw(self, key=None):
        ...
        if self.edit:
            with st.expander(t('agents.agent_label', role=self.role), expanded=True):
                with st.form(key=form_key):
                    self.role = st.text_input(t('agents.role'), value=self.role)
                    self.backstory = st.text_area(t('agents.backstory'), value=self.backstory)
                    self.goal = st.text_area(t('agents.goal'), value=self.goal)
                    self.allow_delegation = st.checkbox(t('agents.allow_delegation'), value=self.allow_delegation)
                    self.verbose = st.checkbox(t('agents.verbose'), value=self.verbose)
                    self.cache = st.checkbox(t('agents.cache'), value=self.cache)
                    self.llm_provider_model = st.selectbox(t('agents.llm_provider'), options=llm_providers_and_models(), index=llm_providers_and_models().index(self.llm_provider_model))
                    self.temperature = st.slider(t('agents.temperature'), value=self.temperature, min_value=0.0, max_value=1.0)
                    self.max_iter = st.number_input(t('agents.max_iterations'), value=self.max_iter, min_value=1, max_value=100)
                    enabled_tools = [tool for tool in ss.tools]
                    tools_key = f"{self.id}_tools_{key}" if key else f"{self.id}_tools"
                    selected_tools = st.multiselect(
                        t('agents.select_tools'),
                    ...
                    if 'knowledge_sources' in ss and len(ss.knowledge_sources) > 0:
                        ...
                        selected_knowledge_sources = st.multiselect(
                            t('agents.knowledge_sources'),
                            options=knowledge_source_options,
                            default=valid_knowledge_sources,
                            format_func=lambda x: knowledge_source_labels.get(x, t('agents.unknown')),
                            key=ks_key
                        )
                        self.knowledge_source_ids = selected_knowledge_sources
                    submitted = st.form_submit_button(t('common.save'))
                    if submitted:
            ...
            with st.expander(expander_title, expanded=False):
                st.markdown(f"**{t('agents.role')}:** {self.role}")
                st.markdown(f"**{t('agents.backstory')}:** {self.backstory}")
                st.markdown(f"**{t('agents.goal')}:** {self.goal}")
                st.markdown(f"**{t('agents.allow_delegation')}:** {self.allow_delegation}")
                st.markdown(f"**{t('agents.verbose')}:** {self.verbose}")
                st.markdown(f"**{t('agents.cache')}:** {self.cache}")
                st.markdown(f"**{t('agents.llm_provider')}:** {self.llm_provider_model}")
                st.markdown(f"**{t('agents.temperature')}:** {self.temperature}")
                st.markdown(f"**{t('agents.max_iterations')}:** {self.max_iter}")
                st.markdown(f"**{t('agents.tools_label')}** {[self.get_tool_display_name(tool) for tool in self.tools]}")
                ...
                if self.knowledge_source_ids and 'knowledge_sources' in ss:
                    ...
                    if knowledge_sources:
                        st.markdown(f"**{t('agents.knowledge_sources')}:**")
                ...
                with col1:
                    ...
                    st.button(t('common.edit'), on_click=self.set_editable, args=(True,), key=btn_key)
                with col2:
                    del_key = f"del_btn_{rnd_id()}"
                    st.button(t('common.delete'), on_click=self.delete, key=del_key)
