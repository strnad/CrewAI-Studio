...
- **LLM providers supported**: Currently OpenAI, Groq, Anthropic, ollama, Grok, OrcaRouter and LM Studio backends are supported. OpenAI key is probably still needed for embeddings in many tools. Don't forget to load an embedding model when using LM Studio.
